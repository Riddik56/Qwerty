...
export const Route = createFileRoute("/test")({
  beforeLoad: async () => {
    ...
    const results = (await getMyModuleResultsFn({ data: { contentType: "test" } })) as Array<{
    ...
    return { enabled: access.filter((x) => x.content_type === "test" && x.is_enabled === 1), results };
  },
  component: TestPage,
});
...
function TestPage() {
  ...
  const [moduleScores, setModuleScores] = useState<Record<string, { score: number; total: number }>>(() => {
    ...
    for (const r of results) {
      next[`${r.direction_key}-m-${r.module_index}`] = { score: r.score, total: r.total_questions };
    }
    return next;
  });
  ...
  const modules = useMemo(() => {
    ...
    return theoryBlocks.map((block, idx) => ({
      id: `${selected.direction!.key}-m-${idx}`,
      title: block.title,
  ...
  const submit = () => {
    ...
    if (answeredCount < questions.length) {
      toast.error("Ответьте на все вопросы перед завершением теста");
      return;
    }
    ...
    <PageShell>
      <section className="mx-auto max-w-7xl">
        <div className="relative overflow-hidden rounded-[2rem] bg-slate-950 p-8 text-white shadow-2xl shadow-slate-400/30 sm:p-10">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_18%,rgba(59,130,246,0.42),transparent_30%),radial-gradient(circle_at_90%_0%,rgba(20,184,166,0.28),transparent_30%),linear-gradient(135deg,#020617,#0f172a_58%,#172554)]" />
          <div className="relative">
            ...
            <h1 className="mt-5 font-display text-4xl font-extrabold sm:text-5xl">Промежуточный тест</h1>
            <p className="mt-4 max-w-3xl text-slate-200">
              Выберите направление и пройдите тематический тест. Прогресс и результат считаются автоматически.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto mt-8 grid max-w-7xl gap-6 lg:grid-cols-[320px_1fr]">
        <aside className="rounded-[2rem] border border-white/70 bg-white/85 p-5 shadow-xl shadow-slate-200/60 backdrop-blur">
          ...
          {available.length === 0 ? (
            <p className="mt-4 text-sm text-muted-foreground">Преподаватель пока не открыл вам доступ к тестам.</p>
          ) : (
        ...
        <div className="rounded-[2rem] border border-white/70 bg-white/90 p-6 shadow-xl shadow-slate-200/60 backdrop-blur sm:p-8">
          {selected ? (
              ...
              <div className="mt-6 grid gap-4">
                {submitted && score !== null && (
                  <div className="rounded-2xl border border-slate-200 bg-white p-4">
                    <div className="font-semibold">Результат теста: {score}/{questions.length}</div>
            ...
            <p className="text-muted-foreground">Нет доступных направлений для тестирования.</p>
          )}
        </div>
      </section>
    </PageShell>
